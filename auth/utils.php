...
function load_database($database_to_load) {
    if (file_exists($database_to_load)) { // Check if the selected database already exists
        ...
        if (is_writable(".")) { // Check if the current directory is writable by PHP before trying to create the database file.
            ...
            echo "<p class='error'>The " . $instance_name . " folder is not writable by PHP. This is a technical error. Please contact a site admin to make them aware of this issue.</p>"; // Throw an error if the authentication directory isn't writable by PHP.
            exit();
        }
    }
}
